async function getdata(lat,lon){  
    let url1=`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&appid=374b929e9dbb926db0761ca76e817fc9`;
...
async function search_val(){
    ...
    let url=`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&APPID=374b929e9dbb926db0761ca76e817fc9`
    try{
        let res=await fetch(url)
        let data=await res.json()
...
function display(data,name){
    ...
    wind.innerHTML=`Wind Speed: ${data.current.wind_speed}km/h`
